...
- Added ASCII art (Otter) and "LONTRIUM CONTROL" branding to the PowerShell installer (`Start-ClaimerControl.ps1`).
- Support for automatically claiming free base games when a Free-to-Keep DLC is promoted on Steam.

### Changed
- **Economy Mode** is now enabled by default.
- Ignore Docker Compose image pull failures during local installer startup.
- Improved the UI layout and spacing of the Schedule configuration in the dashboard.
- Removed outdated Shopee manual login instructions from localization files.
